...
The first refresh from a release that predates this settings screen also reads
the installed `custom/kamp_settings.cfg` before replacing it. Direct legacy
edits are imported into `custom/overrides.cfg`, where subsequent KAMP updates
preserve them.
