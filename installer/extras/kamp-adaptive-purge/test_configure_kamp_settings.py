...
class ConfigureKampSettingsTests(unittest.TestCase):
    ...
    def test_legacy_installed_values_are_imported_before_defaults_refresh(self):
        defaults = dict(self.values)
        legacy = {
            "variable_purge_margin": "9",
        ...
        effective = settings.effective_values(defaults, legacy, {})

        self.assertEqual(effective["variable_purge_margin"], "9")
        self.assertEqual(effective["variable_purge_amount"], "26")
        self.assertEqual(effective["variable_flow_rate"], "11")
        self.assertEqual(effective["variable_stock_purge_fallback"], "0")

    def test_existing_overrides_win_over_legacy_installed_values(self):
        defaults = dict(self.values)
        legacy = {"variable_purge_margin": "9"}
        existing = {"variable_purge_margin": "14"}

        effective = settings.effective_values(defaults, legacy, existing)

        self.assertEqual(effective["variable_purge_margin"], "14")
