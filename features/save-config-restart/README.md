...
If the stock restart reaches either the specifically validated `key798`
extruder-motor (`e`) connection failure or the exact `key1` `Internal error
during ready callback: No active exception to reraise` failure, the worker lets
shutdown settle and requests one firmware restart, matching the recovery
confirmed on hardware. It records the captured error in
`/tmp/k2-save-config-restart.error.log`. Any other error remains fail-closed and
requires manual inspection and a power cycle.
