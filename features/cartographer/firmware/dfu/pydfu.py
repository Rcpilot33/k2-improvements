...
_firmware_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, os.path.join(_firmware_root, "deps"))
...
_backend = usb.backend.libusb1.get_backend(
    find_library=lambda name: "/usr/lib/libusb-1.0.so.0"
)
_usb_find = usb.core.find
usb.core.find = lambda *args, **kwargs: _usb_find(
    *args,
...
def main():
    ...
    parser.add_argument("--binary", help="raw binary file to write", default=False)
    parser.add_argument(
        "--address", help="address for --binary", type=lambda x: int(x, 0), default=0x08000000
    )
    ...
    if args.binary:
        ...
        print("Writing raw binary at 0x%08x..." % args.address)
        write_elements(elements, False, progress=cli_progress)
        print("Exiting DFU...")
        exit_dfu()
        command_run = True
