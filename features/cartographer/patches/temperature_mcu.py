...
class PrinterTemperatureMCU:
    def __init__(self, config):
        ...
        self.printer.register_event_handler("klippy:mcu_identify",
                                            self._mcu_identify)
        # Complete skipped sensor setup before reconnect builds MCU config.
        mcu = self.mcu_adc.get_mcu()
        if hasattr(mcu, 'is_non_critical') and mcu.is_non_critical:
            mcu_name = mcu.get_name()
            reconnect_event = "non_critical_mcu_%s:identified" % mcu_name
            self.printer.register_event_handler(
                reconnect_event, self._mcu_identify)
